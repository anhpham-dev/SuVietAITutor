...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, labels }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-history-ink/40 backdrop-blur-sm p-3 sm:p-4 animate-fade-in no-print">
      <div className="glass-panel rounded-lg shadow-2xl max-w-md w-full overflow-hidden flex flex-col animate-fade-in-up border border-history-gold/20 max-h-[90vh]">

        {/* Header */}
        <div className="bg-white/80 px-4 sm:px-6 py-3 sm:py-4 border-b border-history-gold/10 flex justify-between items-center backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <Key className="w-4 h-4 text-history-red" />
            <h3 className="text-base sm:text-lg font-display font-medium text-history-ink tracking-wide">
              {labels.settings.title}
            </h3>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-black/5 rounded-full text-history-wood transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-8 overflow-y-auto">
          <div className="mb-4 sm:mb-6">
            <h4 className="text-xs sm:text-sm font-bold uppercase tracking-widest text-history-wood mb-2 border-b border-history-gold/10 pb-2">
              {labels.settings.apiSection}
            </h4>
            <p className="text-history-ink font-serif text-xs sm:text-sm leading-relaxed mb-3 sm:mb-4">
        ...
        {/* Footer */}
        <div className="p-3 sm:p-4 bg-history-parchment/20 border-t border-history-gold/10 flex justify-end backdrop-blur-sm">
          <button
            onClick={onClose}
            className="px-4 sm:px-6 py-2 text-history-wood hover:text-history-dark font-medium transition-colors text-[10px] sm:text-xs uppercase tracking-widest"
          >
