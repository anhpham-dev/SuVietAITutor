...
export const StudyMode: React.FC<StudyModeProps> = ({ data, labels }) => {
  ...
  return (
    <div className="animate-fade-in-up max-w-4xl mx-auto pb-10 px-2 sm:px-0">
      <div className="flex justify-center mb-6 sm:mb-10">
        <div className="inline-flex glass-panel p-1 rounded-lg shadow-inner-warm border border-history-gold/10 w-full sm:w-auto">
          <button
            onClick={() => setActiveTab(StudyTab.FLASHCARDS)}
            className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 rounded font-medium text-[10px] sm:text-xs uppercase tracking-widest transition-all duration-300 ${activeTab === StudyTab.FLASHCARDS
          ...
          <button
            ...
            className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 rounded font-medium text-[10px] sm:text-xs uppercase tracking-widest transition-all duration-300 ${activeTab === StudyTab.QUIZ
                ? 'bg-white text-history-ink shadow-sm'
                : 'text-history-wood/60 hover:text-history-dark hover:bg-white/40'
              }`}
          >
...
const InteractiveFlashcards: React.FC<{ data: HistoryLessonData; labels: UILabels }> = ({ data, labels }) => {
  ...
  return (
    <div className="max-w-xl mx-auto px-2 sm:px-4">
      {/* Card Container */}
      <div
        className="relative h-64 sm:h-80 md:h-96 w-full cursor-pointer perspective-1000 mb-6 sm:mb-8 group"
        onClick={() => setIsFlipped(!isFlipped)}
      >
        <div className={`relative w-full h-full transition-transform duration-700 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>

          {/* Front */}
          <div className="absolute w-full h-full backface-hidden glass-card rounded-lg shadow-paper flex flex-col items-center justify-center p-6 sm:p-10 text-center relative overflow-hidden group-hover:shadow-paper-hover transition-shadow duration-300 border border-history-gold/20">
            <div className="absolute top-0 w-full h-1 bg-history-red/80"></div>
            <span className="absolute top-4 sm:top-6 right-4 sm:right-6 text-xs font-bold text-history-wood/30 font-sans">
              #{currentIndex + 1}
            </span>

            <p className="font-display font-medium text-xl sm:text-2xl md:text-3xl text-history-ink leading-snug">
          ...
          {/* Back */}
          <div className="absolute w-full h-full backface-hidden bg-history-ink rounded-lg shadow-paper p-6 sm:p-10 flex items-center justify-center text-center rotate-y-180 relative">
            <div className="absolute inset-2 sm:inset-3 border border-history-gold/20 rounded-sm"></div>
            <p className="text-history-paper text-base sm:text-lg md:text-xl font-serif leading-relaxed font-light">
      ...
      {/* Controls */}
      <div className="flex items-center justify-between px-4 sm:px-6 bg-white/40 backdrop-blur-sm rounded-full p-2 border border-history-gold/10 shadow-sm max-w-xs sm:max-w-sm mx-auto">
        <button
          onClick={(e) => { e.stopPropagation(); handlePrev(); }}
          className="p-1.5 sm:p-2 rounded-full hover:bg-white text-history-wood/70 hover:text-history-dark transition-all active:scale-95"
          title={labels.study.prev}
        >
          <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
        </button>

        <div className="text-[10px] sm:text-xs font-bold text-history-dark font-display tracking-widest">
                ...
                {q.options.map((opt, oIdx) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border transition-colors shrink-0 ${submitted && oIdx === q.correctAnswerIndex ? 'bg-green-700 border-green-700 text-white' :
